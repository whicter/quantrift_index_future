...
> ⚠️ **NQ 金字塔原则：高周期重仓、低周期轻仓**。risk_pct 须与 ATR 配合，才能产生正确的手数分层（1D > 4H > 1H）。若 risk_pct 增幅不足以抵消 ATR 增幅，会导致低周期反而手数最多（倒置金字塔）。
...
> ⚠️ **小账户风险失真警告**：公式计算结果 < 1 手时强制取 1 手最小值，导致高周期的实际风险% 远超意图的 1%。$32k 账户下各 TF 实际风险：
>
> | 周期 | 意图风险 | 实际止损 | 实际风险% |
> |------|---------|---------|---------|
> | NQ 1H | $320（1%）| $240 | 0.75% ✅ |
> | NQ 4H | $320（1%）| $600 | 1.875% ⚠️ |
> | NQ 1D | $320（1%）| $1,200 | 3.75% ❌ |
> | ES 4H | $320（1%）| $375 | 1.17% ⚠️ |
> | ES 1D | $320（1%）| $750 | 2.34% ⚠️ |
>
> 最坏情况（5 TF 同时触发止损）：-$3,165 ≈ -9.9%。**不会爆仓，但单笔 NQ 1D 亏损可达 -3.75%，需有心理准备，不要 panic 干预。** 此系统设计目标为 $100k+ 账户，$32k 下比例失真属正常现象。

**分批止盈（Staged TP，已启用）：**
- TP1：ATR × 1.0，平掉 34% 仓位
- TP2：ATR × 2.0，平掉剩余仓位

> ⚠️ $32k 账户 1 手时 TP1 不生效：`floor(1 × 34%) = 0`，整数截断，实际只有 TP2 触发。

> MES 1H 趋势策略已移除（Sharpe=-0.445）。ES 1H 改由均值回归引擎（`ib-bot-mr`，clientId=21）独立处理，待 `es_mr/mr_engine.py` 开发完成后上线。
