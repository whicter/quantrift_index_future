...
**结论**：15min NQ Range IS Sharpe = -1.685（负），直接排除。三个可行方向：

| 方向 | 描述 | 结论 |
|------|------|------|
| Spread 参数优化 | z_entry/window/max_bars 54组网格搜索 | ✅ 当前参数已最优，不改 |
| ORB 1H | 9:00 bar 定区间，10:00 突破入场 | ❌ 全参数 Sharpe 全为负（-8~-16），无 edge |
| VWAP 15min 回归 | 偏离 VWAP 1.5~2.5×ATR 反向入场 | ❌ IS=1.22 → OOS=0.27，过拟合 78%，不上线 |

**总结：盘整期小级别日内策略在 NQ 上没有可靠 edge。现有 6 bot 组合已最优，无需新增。**

---

### 新策略研究方向（2026-07-02）— 已全部回测完毕

#### 回测结论汇总

| 方向 | Sharpe | IS/OOS | 结论 |
|------|--------|--------|------|
| **IBS 日线（多+空）** | 全部负 | — | ❌ 期货无效：跳空/展期破坏 OHLC 统计规律 |
| **Turn-of-Month NQ** | 0.932 | IS=0.87 / OOS=**0.98** | ⚠️ OOS 稳定但 Sharpe 偏低，每月仅 1 笔，不上线 |
| **Turn-of-Month ES** | 0.880 | IS=1.34 / OOS=0.49 | ❌ OOS 衰减 64%，不稳定 |
| **ES RTH Gap reversion** | 1.169 | IS=**0.14** / OOS=2.27 | ❌ IS 近零（反向过拟合），TP 填满率仅 48% |

**总结：三个方向全部不上线。现有 6 bot 组合已是 NQ/ES 期货策略空间的最优配置。**

#### IBS 排除原因
IBS 文献结果（WR 74%，Sharpe 1.7）基于**现货 ETF（SPY/QQQ）**，期货日线因跳空（维护中断）和展期（换月）导致 OHLC 统计规律失效。NQ/ES 全参数 Sharpe 均为负。

#### Turn-of-Month 排除原因
NQ OOS 虽然稳定（0.98），但 Sharpe < 1、每月仅 1 笔、盈利 4/7 年（翻硬币水平），统计功效太低。ES OOS 衰减 64%，不稳定。

#### ES RTH Gap 排除原因
NQ gap 策略有效是因为维护中断窗口（16:00-17:00）极短、流动性极低，gap 必然填满（100% fill）。ES 早盘 RTH gap 跨越整夜，TP 填满率仅 48%，逻辑根本不同，无法复制。

#### 辅助数据研究

| 数据 | 对 NQ/ES 有用吗 | 获取方式 | 可回测 |
|------|----------------|---------|--------|
| **GEX（Gamma Exposure）** | ✅ 高。正 GEX→区间（利 MR），负 GEX→趋势（利 trend bot） | SqueezeMetrics 免费 CSV | ✅ 历史数据至 2011 |
| **COT（CFTC 持仓报告）** | ✅ 中。大型投机者净多在 90% 分位→偏空；10% 分位→偏多 | CFTC 官网免费，1986 年至今 | ✅ 周频，适合月线 regime |
| 暗盘成交 | ❌ 期货全部中央撮合，无暗盘 | N/A | N/A |
| Block Trade | ⚠️ CME 有报告，零散难自动化 | CME 官网 | 困难 |

**GEX 整合思路**（低优先级，暂不回测）：
- 正 GEX regime → 增加 NQ Range / MR 权重
- 负 GEX regime → 增加 trend bot 权重，暂停 MR
- GEX 穿零（gamma flip）→ 预警波动扩张，等 ADX 确认再入场
- SpotGamma（付费）提供逐 strike 的 GEX 分布；SqueezeMetrics 免费版够做 regime 回测

**COT 回测可行性**：CFTC 历史 CSV 免费，信号简单（净多分位数），但周频+3天滞后，只适合月线 regime 过滤，不适合日内。

#### 已排除方向（完整列表）
- 隔夜漂移（Overnight Drift）：NY Fed 2026 年论文确认 2021 年后归零，已被套利消灭
- ORB 1H：全参数 Sharpe 全为负（-8~-16），NQ 开盘无法产生有效突破
- VWAP 15min 回归：IS=1.22 → OOS=0.27，过拟合 78%
- IBS 日线（多+空）：期货 OHLC 统计规律失效，全参数负
- Turn-of-Month：Sharpe < 1，统计功效不足
- ES RTH Gap：IS 近零，TP 填满率 48%，与 NQ gap 逻辑根本不同
- RSI 多周期波段（PDF）：思路与现有 MR 策略高度重叠，无增量 edge

### Spread 参数敏感性分析结论（2026-07-01）

测试矩阵：z_entry(1.6/1.8/2.0) × window(120/180/240) × max_bars(6/8/10) × z_exit(0.3/0.5)，共 54 组。

**结论：当前参数已在 Pareto 最优区，不改任何参数。**

| 参数组合 | 笔数/月 | WR | Total | Sharpe | OOS衰减 |
|---------|--------|-----|-------|--------|---------|
| z=2.0 w=180 mb=6 | 20.2 | 86.7% | $136k | 51.0 | 6% |
| **z=2.0 w=240 mb=8（当前）** | 20.9 | 86.2% | **$141k** | 49.7 | 12% |
| z=1.6 w=180 mb=6 | 31.5 | 85.5% | $185k | 49.0 | 4% |

- 降 z_entry：笔数多但 WR 降，Sharpe 更低
- 改 window=180：胜率几乎不变，盈利反降 $4k，没有实质好处
- 当前参数（z=2.0 / window=240 / max_bars=8 / z_exit=0.5）保持不变
