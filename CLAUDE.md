...
| 方向 | 理由 | 难度 | 状态 |
|------|------|------|------|
| **IBS 短空（日线）** | 填补空缺（现有 MR 全长多）、无参数、30年证据、Sharpe≈1.7 | 低 | 待回测 |
| **Turn-of-Month MES** | 60年数据，养老金 rebalancing 结构性驱动，WR 62%，每年仅 12 笔 | 极低 | 待回测 |
| **ES RTH Gap reversion** | NQ Gap 已验证（WR 91.2%），ES RTH gap 逻辑相同可复制 | 低 | 待回测 |

**IBS（Internal Bar Strength）公式**：`IBS = (Close - Low) / (High - Low)`，日线
- IBS < 0.20 → 做多（收盘贴近当日低点）
- IBS > 0.80 → 做空（收盘贴近当日高点，regime 过滤：价格在 200MA 以下）
- 参考结果：WR 74%，Sharpe 1.7，30年无参数过拟合

**Turn-of-Month 规则**：每月倒数第 5 个交易日收盘买入 MES，月初第 3 个交易日卖出，持有约 8 天。根因：养老金 401k 扣款 → 月末集中买入 + window dressing + 期权到期解套资金回流。
...
- SpotGamma（付费）提供逐 strike 的 GEX 分布，即"量化的支撑阻力"；SqueezeMetrics 免费版够做 regime 回测

**COT 回测可行性**：CFTC 历史 CSV 免费，信号简单（净多分位数），但周频+3天滞后，只适合周线/月线 regime 过滤，不适合日内。

#### 已排除方向
- 隔夜漂移（Overnight Drift）：NY Fed 2026 年最新论文确认 2021 年后归零，已被套利消灭
