...
### NQ Range（nq_range）— ✅ 回测完成，待写引擎

**策略**：MNQ 1H 区间交易，CI+ADX 双过滤，BB下轨买入，BB中轨止盈
**clientId 候选**：23
**最优参数**：ci_threshold=55, adx_threshold=25, rsi_os=45, sl_mult=1.5, max_bars=16
...
IS/OOS：OOS 胜率反升至 67.9%，无过拟合

---

### NQ/ES 价差（nq_es_spread）— ✅ 研究完成，待执行验证
...
IS/OOS：均 85% WR，25个月 0 亏损。⚠️ 先纸交易验证执行可行性
...
| 横盘震荡（CI>55，ADX<25）| NQ 区间交易（待上线）|
| NQ/ES 短期背离 | NQ/ES 价差（待验证）|
