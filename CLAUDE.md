...
### NQ 隔夜跳空回归（gap_reversion）— ✅ 回测完成，优化中
...
| IS/OOS | 2.329 / 2.257（极稳定）|

**待优化**：周末gap、ATR止损、VIX过滤 → 优化后写引擎
...
| 横盘震荡（CI>55，ADX<25）| NQ 区间交易（ib-bot-nq-range，待干跑）|
| 每日维护中断跳空（>75pts）| NQ 跳空回归（待引擎）|
